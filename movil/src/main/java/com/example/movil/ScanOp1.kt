...
class ScanOp1 : AppCompatActivity() {

    private val tag = "--- ScanOp1 ---"
    ...
    private fun startScanning(newDocPath : String){
        ...
        val scanFile = File(newDocPath, "EscaneoConNombre.pdf") //TODO NOMBRE AQUI
        ...
        chosenScanner.scan(newDocPath, chosenTicket, object : ScanCapture.ScanningProgressListener{
            override fun onScanningPageDone(p0: ScanPage?) {
                Toast.makeText(this@ScanOp1, "Pagina escaneada", Toast.LENGTH_LONG).show()
                Log.d(tag, "Pagina Escaneada")
            }

            override fun onScanningComplete() {
                Toast.makeText(this@ScanOp1, "Escaneo completado", Toast.LENGTH_LONG).show()
            ...
            override fun onScanningError(theException: ScannerException?) {
                try{
                    //Toast.makeText(this@ScanOp1, "Error en el escaneo", Toast.LENGTH_LONG).show()
